...
<!-- Link back to Colorlib can't be removed. Template is licensed under CC BY 3.0. -->
			...
			<script>
								// Function to toggle filter based on contract type
								function toggleFilter(contractType) {
									var element = document.getElementById('contractTypeList').getElementsByTagName('a');
									for (var i = 0; i < element.length; i++) {
										if (element[i].innerHTML.toLowerCase() === contractType.toLowerCase()) {
											if (element[i].classList.contains('active')) {
												// Remove the filter if already active
												element[i].classList.remove('active');
												// Here you can reload the page or fetch all jobs again without filtering
											} else {
												// Add 'active' class to the clicked contract type
												element[i].classList.add('active');
												// Here you can fetch jobs based on the selected contract type
												// and display them accordingly using AJAX
											}
										} else {
											// Remove 'active' class from other contract types
											element[i].classList.remove('active');
										}
									}
								}
